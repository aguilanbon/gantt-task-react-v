...
const TaskGanttContentInner: React.FC<TaskGanttContentProps> = props => {
  ...
  const [renderedTasks, renderedArrows, renderedSelectedTasks] = useMemo(() => {
    ...
    // Pre-compute connection totals per task-side for arrow distribution
    // Key: `${taskId}|${side}|${direction}`
    ...
    for (const [, dependenciesByLevel] of dependencyMap) {
      for (const [taskId, dependencies] of dependenciesByLevel) {
        for (const dep of dependencies) {
          if (!visibleTasksMirror[dep.source.id]) continue;
    ...
    const getConnectionIndex = (key: string): number => {
      const idx = connectionIndices.get(key) || 0;
      connectionIndices.set(key, idx + 1);
      return idx;
    };
              ...
              arrowsRes.push(
                ...
                >
                  <Arrow
                    ...
                    isActive={
                      activeArrowKey === `${source.id}|${taskId}` ||
                    ...
                    toTotalConnections={
                      connectionTotals.get(`${taskId}|${ownTarget}|in`) || 1
                    }
                  />
                </svg>
              );
              ...
              arrowsRes.push(
                ...
                >
                  <Arrow
                    ...
                    onArrowClick={onArrowClick}
                    isActive={
                      activeArrowKey === `${taskId}|${dependent.id}` ||
                      activeTaskId === taskId ||
                      activeTaskId === dependent.id
                    }
                    fromConnectionIndex={getConnectionIndex(
                      `${taskId}|${ownTarget}|out`
                    )}
                    fromTotalConnections={
                      connectionTotals.get(`${taskId}|${ownTarget}|out`) || 1
                    }
                    toConnectionIndex={getConnectionIndex(
